...
enum DateFieldMode { single, range }
...
class DateField extends StatelessWidget {
  ...
  const DateField({
    ...
    this.date,
    this.onPick,
    this.mode = DateFieldMode.single,
    this.dateFrom,
    this.dateTo,
    this.onPickFrom,
    this.onPickTo,
      ...
      child: InkWell(
        onTap: () async {
          ...
          if (mode == DateFieldMode.single) {
            ...
            final picked = await showModalBottomSheet<Map<String, DateTime>>(
              context: context,
              isScrollControlled: true,
              backgroundColor: Colors.transparent,
              builder: (_) => _DateRangePickerBottomSheet(
            ...
            if (picked != null) {
              onPickFrom?.call(picked['from']!);
              onPickTo?.call(picked['to']!);
            }
          }
        },
...
class _DateRangePickerBottomSheetState
  ...
  void initState() {
    ...
    final DateTime base = widget.initialDateFrom ?? widget.firstDate;
    _visibleMonth = DateTime(base.year, base.month, 1);
  }
            ...
            : () {
                setState(() {
                  if (_selectedDateFrom == null ||
                    ...
                    if (current.isBefore(_selectedDateFrom!)) {
                      _selectedDateTo = _selectedDateFrom;
                      _selectedDateFrom = current;
                    } else {
                      _selectedDateTo = current;
                    }
                  }
                });
              },
  ...
  Widget _buildConfirmButton(BuildContext context) {
    final bool canSubmit = _selectedDateFrom != null && _selectedDateTo != null;
    return Padding(
      padding: EdgeInsets.fromLTRB(
        20,
        0,
  ...
  bool _isInRange(DateTime date) {
    if (_selectedDateFrom == null || _selectedDateTo == null) return false;
    return date.isAfter(_selectedDateFrom!) && date.isBefore(_selectedDateTo!);
  }
